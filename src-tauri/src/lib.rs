...
const TIDELINE_PIPEWIRE_FILE: &str = "50-tideline.conf";
const TIDELINE_PULSE_ROUTING_FILE: &str = "50-tideline-routing.conf";
const TIDELINE_WIREPLUMBER_LEGACY_FILE: &str = "50-tideline-routing.conf";
const LEGACY_PIPEWIRE_FILES: &[&str] = &["10-virtual-sinks.conf", "20-loopbacks.conf"];
...
fn wireplumber_conf_dir() -> PathBuf {
    home_dir().join(".config/wireplumber/wireplumber.conf.d")
}

fn pulse_conf_dir() -> PathBuf {
...
fn output_channel(name: &str) -> ChannelCfg {
    let s = slug(name);
    ChannelCfg {
        name: name.into(),
...
fn migrate_config(cfg: &mut AppConfig, raw: &str) {
...
fn mix_playback_node(ch: &ChannelCfg, mix: &Mix, sink_idx: usize) -> String {
    format!("playback.{}-{}-{}", slug(&ch.name), mix.id, sink_idx)
}

fn mix_capture_node(ch: &ChannelCfg, mix: &Mix, sink_idx: usize) -> String {
...
fn sink_node_for_channel(ch: &ChannelCfg) -> String {
    ch.hp_node
...
fn generate_pipewire_config(cfg: &AppConfig) -> Result<String, String> {
...
fn generate_app_routing_config(cfg: &AppConfig) -> String {
    let mut out = String::new();
    out.push_str("# Generated by Tideline — do not edit manually\n\n");
...
fn write_app_routing(cfg: &AppConfig) -> Result<(), String> {
...
fn write_pipewire_conf(cfg: &AppConfig) -> Result<Vec<String>, String> {
