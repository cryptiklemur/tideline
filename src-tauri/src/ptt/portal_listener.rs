...
use crate::ptt::{handle_press, handle_release, handle_toggle, PttRuntime};
use futures_util::StreamExt;
use std::collections::HashMap;
use std::sync::Arc;
use std::time::Duration;
use tauri::AppHandle;
use zbus::{Connection, Proxy, zvariant::{ObjectPath, OwnedObjectPath, OwnedValue, Value}};
...
const SESSION_TOKEN: &str = "tideline_ptt";
const REQ_TOKEN_CREATE: &str = "tideline_ptt_create";
const REQ_TOKEN_BIND: &str = "tideline_ptt_bind";
...
pub async fn try_start(runtime: Arc<PttRuntime>, app: AppHandle) -> Result<(), String> {
    ...
    // CreateSession with a stable session_handle_token.
    ...
    let _create_resp = portal_request(&conn, &portal, "CreateSession", &(create_opts,), &unique_id, REQ_TOKEN_CREATE).await
        .map_err(|e| format!("CreateSession: {}", e))?;

    // Build the session object path that the portal allocated for us.
    let session_path: OwnedObjectPath = ObjectPath::try_from(format!(
        "/org/freedesktop/portal/desktop/session/{}/{}", unique_id, SESSION_TOKEN
    ))
    .map_err(|e| format!("bad session path: {}", e))?
    .into();

    // BindShortcuts. Each shortcut is (id, vardict{description, preferred_trigger}).
    ...
    let parent_window = ""; // empty string ≡ no parent
    ...
    // Spawn the listener. Keep the connection alive in the task so signal
    // streams remain valid for the app lifetime.
    tauri::async_runtime::spawn(async move {
        let _conn = conn.clone(); // hold the connection
            ...
            tokio::select! {
                msg = activated.next() => {
                    ...
                    if let Some(id) = parse_shortcut_id(&msg) {
                        dispatch_activated(&id, &app, &runtime);
                    }
                }
                msg = deactivated.next() => {
                    let Some(msg) = msg else { break };
                    if let Some(id) = parse_shortcut_id(&msg) {
                        dispatch_deactivated(&id, &app, &runtime);
                    }
                }
            }
        }
...
fn dispatch_activated(id: &str, app: &AppHandle, runtime: &Arc<PttRuntime>) {
    match id {
        SHORTCUT_TOGGLE => handle_toggle(app, runtime),
        SHORTCUT_HOLD => handle_press(app, runtime),
        _ => {}
    }
}

fn dispatch_deactivated(id: &str, app: &AppHandle, runtime: &Arc<PttRuntime>) {
    if id == SHORTCUT_HOLD {
        handle_release(app, runtime);
    }
}
