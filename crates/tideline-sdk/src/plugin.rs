...
use crate::rpc::{Notification, Request, Response, RpcError, error_codes};
...
pub async fn run<P: Plugin>(plugin: P) {
    ...
    let p = plugin.clone();
    let h = host.clone();
    tokio::spawn(async move { p.on_ready(h).await; });

    let p_req = plugin.clone();
    let h_req = host.clone();
    let req_task = tokio::spawn(async move {
        while let Some((Request { id, method, params, .. }, ack)) = requests.recv().await {
            let h = h_req.clone();
            let p = p_req.clone();
