...
mod tests {
    ...
    fn honors_xdg_overrides() {
        ...
        assert_eq!(
            plugin_install_dir("io.test"),
            PathBuf::from("/tmp/wave1-xdg-data/tideline/plugins/io.test")
        );
        assert_eq!(
            plugin_log_path("io.test"),
            PathBuf::from("/tmp/wave1-xdg-state/tideline/plugins/io.test/plugin.log")
        );
        assert_eq!(
            plugin_permissions_path("io.test"),
            PathBuf::from("/tmp/wave1-xdg-config/tideline/plugins/io.test/permissions.toml")
        );
    }
}
