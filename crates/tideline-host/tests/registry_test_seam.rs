...
async fn register_multi_plugin_merges_into_global() {
    ...
    let ids: Vec<&str> = snap.status_pills.iter()
        .map(|p| p.plugin_id.as_str()).collect();
...
async fn register_each_kind_lands_in_correct_slot() {
    let reg = PluginRegistry::new_for_test();
    reg.register_contribution(
        "p", ContribKind::SettingsSection,
        json!({"surface_id": "s1", "title": "T", "tree": {}}),
    ).await.unwrap();
    reg.register_contribution(
        "p", ContribKind::StatusPill,
        json!({"surface_id": "p1", "label": "L"}),
    ).await.unwrap();
    reg.register_contribution(
        "p", ContribKind::ChannelOverlay,
        json!({
            "surface_id": "o1",
            "placement": "detail",
    ...
    reg.register_contribution(
        "p", ContribKind::IframeSurface,
        json!({"surface_id": "f1", "entry_path": "ui/index.html"}),
    ).await.unwrap();
    reg.register_contribution(
        "p", ContribKind::TrayItem,
        json!({"item_id": "t1", "label": "Quit"}),
    ).await.unwrap();
    reg.register_contribution(
        "p", ContribKind::KeybindAction,
...
async fn register_invalid_payload_returns_error() {
    let reg = PluginRegistry::new_for_test();
    let err = reg.register_contribution(
        "p", ContribKind::StatusPill,
        json!({"surface_id": "s"}),
    ).await.unwrap_err();
    let s = err.to_string();
