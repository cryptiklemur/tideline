...
async fn revocation_notifies_plugin_via_permissions_changed() {
    ...
    for c in [Capability::LogWrite, Capability::EventsPublish, Capability::EventsSubscribe] {
    ...
    let removed = reg.revoke_capability("io.tideline.test", Capability::EventsPublish)
        .await.unwrap();
    assert!(removed, "EventsPublish should have been granted before revoke");

    let transport = runtime.transport().await.expect("transport");
    let observed: Value = tokio::time::timeout(Duration::from_secs(5), async {
        loop {
            let resp = transport
                .call("plugin/last_perms_change", Some(json!({})), Duration::from_secs(2))
                .await
                .expect("call");
            if !resp.is_null() {
                return resp;
    ...
    let granted = observed.get("granted").and_then(|v| v.as_array())
        .expect("granted array")
        .iter()
        .filter_map(|v| v.as_str().map(|s| s.to_string()))
        .collect::<Vec<_>>();
    assert!(!granted.iter().any(|c| c == "events.publish"),
        "events.publish should be revoked, granted={granted:?}");
    assert!(granted.iter().any(|c| c == "log.write"),
