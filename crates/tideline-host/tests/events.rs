...
async fn plugin_event_publish_and_host_event_fanout() {
    ...
    for c in [LogWrite, EventsPublish, EventsSubscribe, ChannelRead, TrayContribute] {
        reg.grant_capability("io.tideline.test", c).await.unwrap();
    }

    let mut bus_rx = reg.bus.register_plugin("test-observer", vec![]).await;
    reg.bus.subscribe("test-observer", "io.tideline.test:smoketest_done").await.unwrap();
    ...
    reg.bus.publish_host("host:channel.changed", json!({"id":"chan-A"})).await;
