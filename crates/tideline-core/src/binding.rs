...
pub enum Modifier { Ctrl, Shift, Alt, Super }
...
fn is_valid_key(key: &str) -> bool {
    ...
    if key.len() == 1 && key.chars().next().unwrap().is_ascii_alphanumeric() { return true; }
    matches!(key,
...
fn canonical_mods(mods: &[Modifier]) -> Vec<Modifier> {
    ...
    for (m, _) in MOD_ORDER {
        if mods.contains(m) && !out.contains(m) { out.push(*m); }
    }
    out
}
...
fn fmt_mods(mods: &[Modifier]) -> String {
    ...
    for (m, name) in MOD_ORDER {
        if mods.contains(m) { parts.push(name); }
    }
    parts.join("+")
}
...
impl Binding {
    pub fn parse(s: &str) -> Result<Self, String> {
        if s.is_empty() { return Err("empty binding".into()); }
        ...
        if is_valid_key(last) {
            return Ok(Binding::Keyboard { mods, key: last.to_string() });
        }
        Err(format!("'{}' is not a recognized key or mouse button", last))
    }
    ...
    pub fn format(&self) -> String {
        match self {
            Binding::Keyboard { mods, key } => {
                ...
                if mods_s.is_empty() { key.clone() } else { format!("{}+{}", mods_s, key) }
            }
            Binding::Mouse { mods, button } => {
                let mods_s = fmt_mods(mods);
                if mods_s.is_empty() { button.clone() } else { format!("{}+{}", mods_s, button) }
            }
        }
    }
}
...
mod tests {
    ...
    fn rt(s: &str) -> String { Binding::parse(s).unwrap().format() }

    #[test] fn keyboard_plain() { assert_eq!(rt("F12"), "F12"); }
    #[test] fn keyboard_one_mod() { assert_eq!(rt("Ctrl+F12"), "Ctrl+F12"); }
    #[test] fn keyboard_three_mods() { assert_eq!(rt("Ctrl+Shift+Alt+F12"), "Ctrl+Shift+Alt+F12"); }
    #[test] fn keyboard_named() { assert_eq!(rt("ScrollLock"), "ScrollLock"); }
    #[test] fn mouse_plain() { assert_eq!(rt("Mouse5"), "Mouse5"); }
    #[test] fn mouse_with_mod() { assert_eq!(rt("Ctrl+Mouse5"), "Ctrl+Mouse5"); }
    #[test] fn mod_order_normalized() {
        assert_eq!(rt("Shift+Ctrl+F1"), "Ctrl+Shift+F1");
    }
    #[test] fn invalid_returns_err() {
